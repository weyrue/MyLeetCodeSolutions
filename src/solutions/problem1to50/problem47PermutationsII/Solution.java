package solutions.problem1to50.problem47PermutationsII;
...
///**
// * 所有不重复的数字轮番交换到最后一位，然后递归（剩下的数字轮番交换到倒数第二位...），以此类推，每完成一圈的交换便是一个结果。
// * 由于交换后会破坏原来排好的顺序，故每次都需要从头检查是否是重复情况。所以排序的意义不大。
// * @author Yi
// * @version 1ms
// * @since 3/5/2020
// */
...
//            if (!shouldBeSwaped(nums, i)) continue;
//            swap(nums, i, endPos);
...
//    }
//
//    private boolean shouldBeSwaped(int[] nums, int k) {
//        for (int i = 0; i < k; i++) {
//            if (nums[i] == nums[k]) return false;
//        }
//        return true;
//    }
//
